...
The script requires three UUID fixtures supplied through `psql` variables. `user_a_id` and `user_b_id` represent ordinary authenticated users. `admin_id` represents an authenticated admin user whose JWT app/user metadata should satisfy admin policies.
...
The scaffold encodes the SEC-03 task requirements as explicit checks. User A must not be able to read User B rows, and User B must not be able to read User A rows. `WITH CHECK` policy behavior must prevent User A from inserting a row owned by User B. Admin-shaped JWT claims must be able to select operational tables. Non-admin users must not see `EmailLog`. Anonymous users must not be able to write `LoadingDockMessage`. `EmailLog` must expose an insert policy surface for the approved service-role/server-side path.
...
| Owner spoof prevention | Inserts that set the owner column to another user fail. |
...
Run the script after applying SEC-02 policies in a disposable branch database, staging database, or local Supabase instance. Do not run it against production until the fixture IDs are known safe and the operator intends to validate production policy posture.
