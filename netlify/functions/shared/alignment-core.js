/**
 * alignment-core.js
 * Extracted core logic for the Notion ↔ SaaS Alignment Loop.
 */
...
async function writeDriftReport(report) {
  ...
  try {
    await timedFetch('https://api.notion.com/v1/pages', {
      method: 'POST',
      headers: notionHeaders(),
...
// ── PAL-21 Application Helpers ──────────────────────────────────────────────
const TUJ_APPLICATION_STAGE_TO_NOTION = {
  'Saved': 'Saved', 'To apply': 'To apply', 'Applied': 'Applied', 'Screening': 'Screening',
  'Interview Scheduled': 'Interview Scheduled', 'Interviewing': 'Interviewing',
...
function normalizeApplicationStatus(application = {}) {
  ...
  switch (String(application.status || '').toLowerCase()) {
    case 'submitted': case 'applied': return 'Applied';
    case 'screening': return 'Screening';
    case 'interview_scheduled': return 'Interview Scheduled';
    case 'interviewing': return 'Interviewing';
    case 'offered': case 'offer': return 'Offer';
    case 'rejected': case 'withdrawn': return 'Rejected';
    case 'archived': return 'Archived';
    default: return 'Saved';
  }
}

function applicationCompany(application = {}) { return application.company_name || application.company || ''; }
function applicationRole(application = {}) { return application.job_title || application.role || application.title || ''; }
function applicationSourceUrl(application = {}) { return application.source || application.job_url || application.url || ''; }

function notionTextProperty(content) { return content ? { rich_text: [{ text: { content: String(content).slice(0, 2000) } }] } : { rich_text: [] }; }
function notionTitleProperty(content) { return { title: [{ text: { content: String(content || 'Untitled application').slice(0, 2000) } }] }; }
function notionStatusProperty(statusName) { return { status: { name: statusName || 'Saved' } }; }
...
function buildNotionApplicationProperties(application) {
  ...
  const properties = {
    ...
    'ATS Score': { number: typeof application.ats_score === 'number' ? application.ats_score : null },
...
async function upsertNotionApplication(application) {
  ...
  if (application.notion_page_id) {
    const res = await timedFetch(`https://api.notion.com/v1/pages/${application.notion_page_id}`, {
      method: 'PATCH',
      headers: notionHeaders(),
      body: JSON.stringify({ properties: buildNotionApplicationProperties(application) }),
    });
    if (res.ok) return { page_id: application.notion_page_id, action: 'updated' };
  }
  // Try URL match
  const sourceUrl = applicationSourceUrl(application);
  if (sourceUrl) {
    ...
    if (byUrl) {
      await timedFetch(`https://api.notion.com/v1/pages/${byUrl.id}`, {
        method: 'PATCH',
        headers: notionHeaders(),
        body: JSON.stringify({ properties: buildNotionApplicationProperties(application) }),
      });
      return { page_id: byUrl.id, action: 'updated' };
    }
  }
  // Create
  const res = await timedFetch('https://api.notion.com/v1/pages', {
    method: 'POST',
    headers: notionHeaders(),
    body: JSON.stringify({ parent: { database_id: dbId }, properties: buildNotionApplicationProperties(application) }),
  });
  const created = await res.json();
  return { page_id: created.id, page_url: created.url, action: 'created' };
}
...
async function syncApplicationsToNotion() {
  ...
  if (!process.env.BASE44_APPLICATION_URL || !process.env.NOTION_JD_PIPELINE_DB_ID) return summary;
  ...
  for (const application of applications) {
    try {
      ...
      summary.application_errors.push({ id: application.id, error: err.message });
    }
  }
  return summary;
}
...
async function runAlignmentLoop() {
  ...
  try {
    ...
    Object.assign(results, appResults);
  } catch (err) {
  ...
  for (const page of passengers) {
    ...
    const saasState = {
      ...
      resume_fit_check_status: b44User?.passport_completed_at ? 'complete' : (b44User?.highest_ats_score > 0 ? 'in_progress' : 'not_started'),
    };
    ...
    for (const [field, policy] of Object.entries(FIELD_POLICY)) {
      ...
      results.drifts_detected++;
      const report = { passenger_id: passengerId, field_name: field, notion_value: notionValue, saas_value: saasValue, repair_action: policy, source: 'passenger', drift_type: 'field' };
